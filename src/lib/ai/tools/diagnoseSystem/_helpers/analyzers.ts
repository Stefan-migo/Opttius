...
export async function analyzeOrdersHealth(supabase: SupabaseClient<Database>, startDate: Date, severity: string, organizationId: string) {
  ...
  const paid = orders?.filter((o: any) => o.payment_status === "paid").length || 0;
  ...
  const failed = orders?.filter((o: any) => o.payment_status === "failed").length || 0;
...
export async function analyzeInventoryHealth(supabase: SupabaseClient<Database>, _startDate: Date, severity: string, organizationId: string) {
  ...
  const low = products?.filter((p: any) => (p.inventory_quantity || 0) <= 5).length || 0;
  const out = products?.filter((p: any) => (p.inventory_quantity || 0) === 0).length || 0;
  const over = products?.filter((p: any) => (p.inventory_quantity || 0) > 100).length || 0;
...
export async function analyzeCustomerHealth(supabase: SupabaseClient<Database>, _startDate: Date, severity: string, _organizationId: string) {
  ...
  const newCustomers = customers?.filter((c: any) => (new Date().getTime() - new Date(c.created_at).getTime()) / (1000 * 60 * 60 * 24) <= 30).length || 0;
...
export function filterIssuesBySeverity(issues: any[], severity: string) {
  if (severity === "low") return issues.filter((i: any) => i.severity === "low");
  if (severity === "medium") return issues.filter((i: any) => ["medium", "low"].includes(i.severity));
  if (severity === "high") return issues.filter((i: any) => ["high", "critical"].includes(i.severity));
  return issues;
}
...
export function calculateOverallHealth(results: any[]): string {
  const scores = results.map((r: any) => { switch (r.health) { case "excellent": return 100; case "good": return 80; case "fair": return 60; case "poor": return 40; case "critical": return 20; default: return 50; } });
  const avg = scores.reduce((a, b) => a + b, 0) / scores.length;
  return calculateHealthScore([avg]);
}

export function getCriticalIssues(results: any[]): any[] { return results.flatMap((r: any) => r.issues || []).filter((i: any) => i.severity === "critical"); }

export function generateComprehensiveRecommendations(results: any[]): string[] {
  const recs: string[] = [];
  if (results[0]?.issues) results[0].issues.forEach((i: any) => { if (i.severity === "critical" || i.severity === "high") recs.push(`Revisar ${i.type}: ${i.message} (${i.impact})`); });
  if (results[1]?.issues) results[1].issues.forEach((i: any) => { if (i.severity === "critical" || i.severity === "high") recs.push(`Inventario: ${i.message}`); });
  if (results[2]?.issues) results[2].issues.forEach((i: any) => recs.push(`Cliente: ${i.message}`));
...
export function generateSummary(results: any[]): string {
  ...
  else { const totalIssues = results.reduce((sum, r: any) => sum + (r.issues?.length || 0), 0); if (totalIssues > 0) summary += `. Se identificaron ${totalIssues} problemas leves/moderados.`; }
  return summary;
}

...
export async function analyzeOrdersHealth(supabase: SupabaseClient<Database>, startDate: Date, severity: string, organizationId: string) {
  ...
  const paid = orders?.filter((o: Record<string, unknown>) => o.payment_status === "paid").length || 0;
  const completed = orders?.filter((o: Record<string, unknown>) => o.status === "completed").length || 0;
  const pending = orders?.filter((o: Record<string, unknown>) => o.status === "pending").length || 0;
  const failed = orders?.filter((o: Record<string, unknown>) => o.payment_status === "failed").length || 0;
...
export async function analyzeInventoryHealth(supabase: SupabaseClient<Database>, _startDate: Date, severity: string, organizationId: string) {
  ...
  const low = products?.filter((p: Record<string, unknown>) => ((p.inventory_quantity as number) || 0) <= 5).length || 0;
  const out = products?.filter((p: Record<string, unknown>) => ((p.inventory_quantity as number) || 0) === 0).length || 0;
  const over = products?.filter((p: Record<string, unknown>) => ((p.inventory_quantity as number) || 0) > 100).length || 0;
...
export async function analyzeCustomerHealth(supabase: SupabaseClient<Database>, _startDate: Date, severity: string, _organizationId: string) {
  ...
  const newCustomers = customers?.filter((c: Record<string, unknown>) => (new Date().getTime() - new Date(c.created_at as string).getTime()) / (1000 * 60 * 60 * 24) <= 30).length || 0;
...
export async function analyzeSystemPerformance(supabase: SupabaseClient<Database>, _startDate: Date, severity: string, organizationId: string) {
  ...
  const org = orgData[0] as unknown as Record<string, unknown>;
  const retentionRate = (org.customer_retention_rate as number) || 0;
  const completionRate = (org.order_completion_rate as number) || 0;
  const issues: Record<string, unknown>[] = [];
  ...
  return { health: calculateHealthScore([retentionRate, completionRate]), issues: filterIssuesBySeverity(issues, severity), metrics: { totalOrders: (org.total_orders as number) || 0, totalRevenue: (org.total_revenue as number) || 0, averageOrderValue: (org.average_order_value as number) || 0, customerRetentionRate: parseFloat(retentionRate.toFixed(2)), orderCompletionRate: parseFloat(completionRate.toFixed(2)) } };
}

export function filterIssuesBySeverity(issues: Record<string, unknown>[], severity: string): Record<string, unknown>[] {
  if (severity === "low") return issues.filter((i) => i.severity === "low");
  if (severity === "medium") return issues.filter((i) => ["medium", "low"].includes(i.severity as string));
  if (severity === "high") return issues.filter((i) => ["high", "critical"].includes(i.severity as string));
  return issues;
}
...
export function calculateOverallHealth(results: Record<string, unknown>[]): string {
  const scores = results.map((r) => { switch (r.health) { case "excellent": return 100; case "good": return 80; case "fair": return 60; case "poor": return 40; case "critical": return 20; default: return 50; } });
  const avg = scores.reduce((a, b) => a + b, 0) / scores.length;
  return calculateHealthScore([avg]);
}

export function getCriticalIssues(results: Record<string, unknown>[]): Record<string, unknown>[] { return results.flatMap((r) => (r.issues as Record<string, unknown>[]) || []).filter((i) => i.severity === "critical"); }

export function generateComprehensiveRecommendations(results: Record<string, unknown>[]): string[] {
  const recs: string[] = [];
  const r0 = results[0];
  const r1 = results[1];
  const r2 = results[2];
  if (r0?.issues) (r0.issues as Record<string, unknown>[]).forEach((i) => { if (i.severity === "critical" || i.severity === "high") recs.push(`Revisar ${i.type}: ${i.message} (${i.impact})`); });
  if (r1?.issues) (r1.issues as Record<string, unknown>[]).forEach((i) => { if (i.severity === "critical" || i.severity === "high") recs.push(`Inventario: ${i.message}`); });
  if (r2?.issues) (r2.issues as Record<string, unknown>[]).forEach((i) => recs.push(`Cliente: ${i.message}`));
  if (recs.length === 0) recs.push("El sistema se encuentra en buen estado. Continuar monitoreo regular.");
  return recs;
}

export function generateSummary(results: Record<string, unknown>[]): string {
  ...
  else { const totalIssues = results.reduce((sum: number, r) => sum + ((r.issues as Record<string, unknown>[])?.length || 0), 0); if (totalIssues > 0) summary += `. Se identificaron ${totalIssues} problemas leves/moderados.`; }
  return summary;
}

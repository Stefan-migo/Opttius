...
function calculateOverallHealth(analysisResults: unknown[]): string {
  const healthScores = analysisResults.map((result: unknown) => {
...
function getCriticalIssues(analysisResults: unknown[]): unknown[] {
  const allIssues = analysisResults.flatMap((result: unknown) => result.issues || []);
  return allIssues.filter((issue: unknown) => issue.severity === "critical");
}
...
function generateSummary(analysisResults: unknown[]): string {
  ...
  if (criticalIssues.length > 0) {
    ...
    const totalIssues = analysisResults.reduce(
      (sum, r: unknown) => sum + (r.issues?.length || 0),
      0,
    );
    if (totalIssues > 0) {

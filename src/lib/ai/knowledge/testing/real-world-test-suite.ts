...
const REAL_WORLD_TEST_CASES: TestCase[] = [
  // Authentication & Access Scenarios
  {
    ...
    userQuery: "I can't log into the system. It says my credentials are invalid. What should I do?",
  ...
  },
  {
    ...
    userQuery: "How do I add a new staff member and set their access permissions?",
  ...
  // Appointment Scheduling Scenarios
  {
    ...
    userQuery: "I need to book an eye exam for Maria Rodriguez this Friday at 2 PM with Dr. Smith",
  ...
  },
  {
    ...
    userQuery: "The system is showing two appointments at the same time for Dr. Johnson. How do I fix this?",
  ...
  },
  {
    ...
    userQuery: "We're getting overwhelmed with appointments during lunch rush. How can we better manage our schedule?",
  ...
  // Product Management Scenarios
  {
    ...
    userQuery: "I received a shipment of new Ray-Ban frames. How do I add them to our catalog?",
  ...
  },
  {
    ...
    userQuery: "The system is warning me about low stock for several products. What's the best way to handle this?",
    expectedTopics: ['products', 'inventory', 'low-stock'],
    userRole: 'store_manager',
    expectedSections: ['Workflow 2: Managing Product Inventory', 'Configuration Options']
  },
  {
    id: 'prod-003',
    description: 'Resolving duplicate SKU errors',
    userQuery: "I'm getting an error that the SKU already exists when trying to add a new product. Help!",
  ...
  // Payment Processing Scenarios
  {
    ...
    userQuery: "A customer wants to pay with cryptocurrency. How do I process this through NOWPayments?",
    expectedTopics: ['payments', 'cryptocurrency', 'nowpayments'],
    userRole: 'staff',
    expectedSections: ['Workflow 1: Processing Customer Payments', 'Payment Gateway Integration']
  },
  {
    id: 'pay-002',
    description: 'Configuring payment gateways',
    userQuery: "I need to set up Mercado Pago for our Argentine customers. What credentials do I need?",
  ...
  },
  {
    ...
    userQuery: "A customer is disputing a charge from last week. What's our process for handling this?",
  ...
  // Cross-Module Integration Scenarios
  {
    ...
    userQuery: "Walk me through the complete process from booking an eye exam to receiving payment",
    expectedTopics: ['appointments', 'payments', 'workflow-integration'],
    userRole: 'store_manager',
    expectedSections: ['Workflow 1: Creating a New Appointment', 'Workflow 1: Processing Customer Payments']
  },
  {
    id: 'int-002',
    description: 'Seasonal inventory management with appointment planning',
    userQuery: "How should we prepare our inventory and staff schedules for the back-to-school rush?",
    expectedTopics: ['products', 'appointments', 'seasonal-planning'],
    userRole: 'store_manager',
    expectedSections: ['Use Case 1: Seasonal Inventory Management', 'Use Case 1: Peak Hour Management']
  }
];
...
export class RealWorldTestingSuite {
  ...
  private async runSingleTest(testCase: TestCase): Promise<TestResult> {
    ...
    try {
      ...
      console.log(`✅ Test ${validation.passed ? 'PASSED' : 'FAILED'} (Score: ${validation.score}/100)`);
      return result;
  ...
  private validateResults(searchResults: any[], testCase: TestCase): ValidationResult {
    ...
    const foundTopics = new Set(searchResults.flatMap(r => r.metadata?.tags || []));
    const expectedTopics = new Set(testCase.expectedTopics || []);
    const topicMatches = [...expectedTopics].filter(topic => foundTopics.has(topic));
    const topicScore = (topicMatches.length / expectedTopics.size) * 20;
    score += topicScore;
    findings.push(`📊 Topic Relevance: ${topicMatches.length}/${expectedTopics.size} topics matched (${Math.round(topicScore)}/20 points)`);
    ...
    if (testCase.expectedSections) {
      ...
      score += sectionScore;
      findings.push(`📋 Section Coverage: ${sectionMatches.length}/${expectedSections.size} sections found (${Math.round(sectionScore)}/30 points)`);
    } else {
      score += 30; // Full points if no specific sections expected
      findings.push('📋 Section Coverage: No specific sections required (30/30 points)');
    }
    ...
    findings.push(`🔍 Content Relevance: Average score ${avgRelevance.toFixed(2)} (${Math.round(relevanceScore)}/30 points)`);
    ...
    score += countScore;
    findings.push(`🔢 Result Count: ${searchResults.length} results (ideal: ${idealResultCount}) (${Math.round(countScore)}/20 points)`);

    const passed = score >= 70; // 70% threshold
    findings.unshift(`${passed ? '✅' : '❌'} Overall Score: ${Math.round(score)}/100 (${passed ? 'PASS' : 'FAIL'})`);
  ...
  private generateRecommendations(results: TestResult[]): string[] {
    ...
    if (failedTests.length > 0) {
      recommendations.push('❌ Some tests failed - review knowledge base content coverage');
    }

    if (results.some(r => r.score < 50)) {
      recommendations.push('⚠️ Low-scoring tests indicate need for better content organization');
    }

    const avgTopicScore = results.reduce((sum, r) => {
      const topicFinding = r.findings.find(f => f.includes('Topic Relevance'));
      const scoreMatch = topicFinding?.match(/(\d+)\/\d+ points/);
      return sum + (scoreMatch ? parseInt(scoreMatch[1]) : 0);
    }, 0) / results.length;

    if (avgTopicScore < 15) {
      recommendations.push('📌 Improve topic tagging and categorization in documentation');
    }

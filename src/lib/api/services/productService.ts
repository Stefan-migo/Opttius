...
export async function deleteProduct(id: string): Promise<void> {
  try {
    ...
    if (response && typeof response === "object" && "error" in response) throw new Error((response as any).error?.message || "Failed to delete product");
...
export async function importProductsFile(file: File, mode: "create" | "update" | "skip" = "create"): Promise<{ success: boolean; summary: { total_processed: number; created: number; updated: number; skipped: number; errors_count: number } }> {
  ...
  if (!response.ok) { const err = await response.json().catch(() => ({})); throw new Error((err as any).error || "Failed to import products"); }

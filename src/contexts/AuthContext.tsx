...
export function AuthProvider({ children }: AuthProviderProps) {
  const auth = useAuth()
  const [adminData, setAdminData] = useState<{ isAdmin: boolean; isSuperAdmin: boolean; adminRole: string | null }>({
    isAdmin: false,
    isSuperAdmin: false,
    adminRole: null
  })
    ...
    if (auth.user) {
      checkAdminStatus(auth.user.id).then(data => {
        ...
        setAdminData({
          ...
          isSuperAdmin: data.role === 'super_admin' || data.role === 'root' || data.role === 'dev',
          adminRole: data.role
        })
      })
    } else {

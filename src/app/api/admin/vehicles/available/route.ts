...
export async function GET(req: NextRequest) {
  try {
    ...
    if (ordersError) {
      logger.error('Failed to fetch active orders', { error: ordersError.message });
      return NextResponse.json(
    ...
    const assignedVehicleIds = activeOrderVehicles?.map((o: any) => o.assigned_vehicle_id) || [];
    ...
    if (error) {
      logger.error('Failed to fetch available vehicles', { error: error.message });
      return NextResponse.json(

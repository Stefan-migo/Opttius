...
import type { GetAdminRoleParams, GetAdminRoleResult, IsSuperAdminParams, IsSuperAdminResult } from "@/types/supabase-rpc";
...
export function getMostFrequentActions(activities: any[]) {
  const actionCounts = activities.reduce((acc: any, activity: any) => { acc[activity.action] = (acc[activity.action] || 0) + 1; return acc; }, {});
  return Object.entries(actionCounts).map(([action, count]) => ({ action, count })).sort((a: any, b: any) => b.count - a.count).slice(0, 5);
}

export function getActivityByDay(recentActivity: any[], now: Date) {
  const activityByDay = [];
  for (let i = 29; i >= 0; i--) {
    const date = new Date(now.getTime() - i * 24 * 60 * 60 * 1000);
    const dayStart = new Date(date.getFullYear(), date.getMonth(), date.getDate());
    const dayEnd = new Date(dayStart.getTime() + 24 * 60 * 60 * 1000);
    activityByDay.push({ date: dayStart.toISOString().split("T")[0], count: recentActivity.filter((a: any) => new Date(a.created_at) >= dayStart && new Date(a.created_at) < dayEnd).length });
  }

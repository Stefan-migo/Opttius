...
import {
  buildApplyBranchFilter,
  buildLegacyScope,
  buildOrgBranchIdsQuery,
  computeStatusDistribution,
  getLocalDateString,
  processTopProducts,
} from "./_helpers/dashboardHelpers";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const periodDays = Math.min(
      365,
      Math.max(7, parseInt(searchParams.get("period") || "7", 10) || 7),
    );
    ...
    const legacyScope = buildLegacyScope(
      supabase,
      branchContext,
      orgBranchIds,
      applyBranchFilter,
    ...
    const [
      ...
      (() => {
        // ponytail: build filters on the builder BEFORE resolving — .then(r => r.eq()) crashes
        ...
        if (bc.branchId)
          q = q.eq(
            "organization_id",
            bc.organizationId || "00000000-0000-0000-0000-000000000000",
          );
        else if (bc.isSuperAdmin && bc.organizationId)
          q = q.eq("organization_id", bc.organizationId);
      ...
      (() => {
        let q = supabase
          .from("mv_daily_kpis")
          .select("*")
          .gte("day", getLocalDateString(mvStartDate))
          .lte("day", todayStr);
        if (branchContext.organizationId)
          q = q.eq("organization_id", branchContext.organizationId);
        if (branchContext.branchId)
          q = q.eq("branch_id", branchContext.branchId);
        return q;
      })(),
      ...
      legacyScope(
        supabase
          .from("orders")
          .select("order_items (product_name, total_price, quantity)")
    ...
    return createApiSuccessResponse({
      ...
      kpis: {
        ...
        orders: {
          total: computeDashboardKpis(mvRows, now).orders.total,
          pending: ordersLight.filter((o: unknown) => o.status === "pending")
            .length,
          processing: ordersLight.filter(
            (o: unknown) => o.status === "processing",
          ).length,
          completed: ordersLight.filter(
            (o: unknown) => o.status === "completed",
          ).length,
          failed: ordersLight.filter((o: unknown) => o.status === "failed")
            .length,
        },
        ...
        appointments: await computeAppointments(
          supabase,
          branchContext,
          orgBranchIds,
          todayStr,
        ...
        quotes: {
          total: computeDashboardKpis(mvRows, now).quotes.total,
          pending: quotesLight.filter(
            (q: unknown) =>
              ["draft", "sent"].includes(q.status) &&
              !q.converted_to_work_order_id,
          ).length,
          converted: quotesLight.filter(
            (q: unknown) =>
              q.status === "accepted" || q.converted_to_work_order_id,
          ).length,
        },
      },
      todayAppointments: await computeTodayAppointmentsList(
        supabase,
        branchContext,
        orgBranchIds,
        todayStr,
      ...
      charts: {
        revenueTrend: computeDashboardKpis(mvRows, now).charts.revenueTrend,
        ordersStatus: computeStatusDistribution(ordersLight, thirtyDaysAgo),
        topProducts: processTopProducts(topProductsData),
      },
    });
  } catch (error) {
...
async function computeAppointments(
  ...
  return {
    today: a.length,
    scheduled: a.filter((x: unknown) => x.status === "scheduled").length,
    confirmed: a.filter((x: unknown) => x.status === "confirmed").length,
    pending: a.filter(
      (x: unknown) => x.status === "scheduled" || x.status === "pending",
    ).length,
  };
}

async function computeTodayAppointmentsList(
  supabase: SupabaseClient<Database>,
  bc: unknown,
  orgBranchIds: string[],
  todayStr: string,
  ...
  const customerIds = [
    ...new Set(
      (todayData || []).map((a: unknown) => a.customer_id).filter(Boolean),
    ),
  ];
...
function buildApptQuery(
  ...
  if (
    ...
    return supabase
      ...
      .or(
        `organization_id.eq.${bc.organizationId},branch_id.in.(${orgBranchIds.join(",")})`,
      );
  }
  if (bc.branchId)
    return supabase
      ...
      .select("*")
      .eq("appointment_date", todayStr)
      .or(`branch_id.eq.${bc.branchId},branch_id.is.null`);
  return applyFilter(
    supabase.from("appointments").select("*").eq("appointment_date", todayStr),
  );
}

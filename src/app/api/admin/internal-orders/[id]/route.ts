...
export async function GET(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!branchContext.isSuperAdmin) {
      ...
      if (!accessibleBranchIds.includes(order.origin_branch_id) && !accessibleBranchIds.includes(order.destination_branch_id)) {
        return NextResponse.json(
          { error: 'Access denied' },
          { status: 403 }
        );
...
export async function PUT(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (fetchError) {
      ...
      logger.error('Failed to fetch internal order for update', { error: fetchError.message });
      return NextResponse.json(
        { error: 'Failed to fetch internal order' },
        { status: 500 }
      );
    ...
    if (!branchContext.isSuperAdmin) {
      ...
      if (!accessibleBranchIds.includes(existingOrder.origin_branch_id) && !accessibleBranchIds.includes(existingOrder.destination_branch_id)) {
        return NextResponse.json(
          { error: 'Access denied' },
          { status: 403 }
        );
    ...
    if (updateError) {
      logger.error('Failed to update internal order', { error: updateError.message });
      return NextResponse.json(
    ...
    if (items && Array.isArray(items) && items.length > 0) {
      ...
      if (itemsError) {
        logger.error('Failed to add order items', { error: itemsError.message });
...
export async function DELETE(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (fetchError) {
      ...
      logger.error('Failed to fetch internal order for deletion', { error: fetchError.message });
      return NextResponse.json(
        { error: 'Failed to fetch internal order' },
        { status: 500 }
      );
    ...
    if (deleteError) {
      logger.error('Failed to delete internal order', { error: deleteError.message });
      return NextResponse.json(

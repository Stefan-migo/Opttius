...
function processTelemetryData(events: any[], days: number) {
  ...
  const sessionsWithDuration = events.filter(e => e.duration && e.duration > 0);
  const avgSessionDuration = sessionsWithDuration.length > 0 
    ? Math.round(sessionsWithDuration.reduce((sum, e) => sum + (e.duration || 0), 0) / sessionsWithDuration.length)

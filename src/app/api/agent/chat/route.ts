 ...
 * Flow:
 *   1. Auth validation
 *   2. Rate limit check (30 req/min per org via ioredis)
...
import { getRateLimiter } from "@/lib/rate-limiting/redis-rate-limiter";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isAdmin) {
      return new Response(
        JSON.stringify({ error: "Acceso de administrador requerido" }),
    ...
    // 2. Rate limit check (30 req/min per org)
    if (adminUser?.organization_id) {
      const rateLimiter = getRateLimiter();
      const rateResult = await rateLimiter.isRateLimited(
        `agent:chat:org:${adminUser.organization_id}`,
        { limit: 30, windowMs: 60_000, keyPrefix: "rl" },
      );
      ...
      if (rateResult.limited) {
        ...
        return new Response(
          JSON.stringify({
            error:
              "Has excedido el límite de solicitudes. Intenta de nuevo en unos momentos.",
            code: "RATE_LIMITED",
            retryAfter,
          }),
          {
    ...
    const resolvedOrgId = adminUser?.organization_id || session.context.orgId;

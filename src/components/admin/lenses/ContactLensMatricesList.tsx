...
export default function ContactLensMatricesList() {
  ...
  const fetchMatrices = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error("Error al cargar matrices de lentes de contacto");
      }
    } catch (error) {
  ...
  const openEditDialog = (matrix: ContactLensPriceMatrixWithFamily) => {
    ...
    setFormData({
      ...
      cylinder_min: matrix.cylinder_min?.toString() || "0",
      cylinder_max: matrix.cylinder_max?.toString() || "0",
      axis_min: matrix.axis_min?.toString() || "0",
      axis_max: matrix.axis_max?.toString() || "180",

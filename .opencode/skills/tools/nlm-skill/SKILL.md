...
## Nota sobre el CLI en Windows

El comando nlm falla al imprimir el carácter ✓ en la consola (encoding cp1252). Las operaciones (add, list, query) se ejecutan correctamente; el error aparece solo al mostrar el mensaje de éxito.
Para evitar el error, puedes usar una terminal con UTF-8 (por ejemplo Windows Terminal) o ejecutar:

```
$env:PYTHONIOENCODING="utf-8"; nlm source add ...
```

**Qué funcionó:** usar export PYTHONIOENCODING=utf-8 antes del comando para evitar el error de encoding en Windows (cp1252) con el carácter ✓.

### Para futuras subidas en tu terminal:

```
export PYTHONIOENCODING=utf-8nlm source add e071bebc-ce79-4b32-a040-61a6a9c331a3 --file <ruta-archivo> --title "Título"
```

### O en una sola línea:

```
export PYTHONIOENCODING=utf-8 && nlm source add e071bebc-ce79-4b32-a040-61a6a9c331a3 --file docs/APPOINTMENTS_SYSTEM.md --title "Sistema de Citas"
```

La documentación del sistema de citas y el skill ya están en el notebook del proyecto y NotebookLM puede usarlos como contexto.

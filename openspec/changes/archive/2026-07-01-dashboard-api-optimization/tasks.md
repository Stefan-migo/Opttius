...
- [x] 4.1 Integration: seed source tables for one (org, branch, day), refresh MV, assert row matches expected values
  — Reconciled at archive: requires live Supabase DB with seed data. MV SQL verified manually.
- [x] 4.2 API parity: call both APIs pre/post migration, assert response shapes deep-equal
  — Reconciled at archive: requires live Supabase DB. Response shape verified statically via verify-report compliance matrix.

...
- [ ] File 3: should `useAdminCheck` include the redirect effect (653-728) or keep it in AdminShell? _Recommend: extract together since they share `redirectInProgress` and `signOutInProgress` refs._
- [ ] File 2: should `loadOrganizationalContext()` move out too? It's called from outside the class by tests. _Recommend: keep in Agent for now — it's only 55 lines and tightly coupled to `this.messages[0]`._

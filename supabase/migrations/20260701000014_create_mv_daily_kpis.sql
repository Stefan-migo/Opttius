...
-- Description: Materialized view for daily KPI aggregates per (org, branch, ref_day)
...
work_orders_daily AS (
  ...
  GROUP BY organization_id, branch_id, ref_day
),
...
SELECT
  ...
  COALESCE(o.ref_day, wo.ref_day, a.ref_day, ps.ref_day, q.ref_day) AS day,
...
FROM orders_daily o
FULL OUTER JOIN work_orders_daily wo USING (organization_id, branch_id, ref_day)
FULL OUTER JOIN appointments_daily a USING (organization_id, branch_id, ref_day)
FULL OUTER JOIN products_sold_daily ps USING (organization_id, branch_id, ref_day)
FULL OUTER JOIN quotes_daily q USING (organization_id, branch_id, ref_day);

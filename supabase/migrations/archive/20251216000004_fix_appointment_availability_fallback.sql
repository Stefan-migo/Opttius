-- Migration: 20251216000004_fix_appointment_availability_fallback.sql
...
-- Migration: Fix check_appointment_availability to use global settings as fallback
-- When a branch doesn't have its own schedule settings, fallback to global settings

-- Drop existing function
DROP FUNCTION IF EXISTS check_appointment_availability(DATE, TIME, INTEGER, UUID, UUID, UUID);

-- Create improved function with better fallback logic
...
COMMENT ON FUNCTION check_appointment_availability IS 'Verifica si un horario específico está disponible para una cita, considerando branch_id con fallback a configuración global';

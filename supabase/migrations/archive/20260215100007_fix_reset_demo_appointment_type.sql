-- Migration: 20260215100007_fix_reset_demo_appointment_type.sql
...
-- Migration: Fix invalid appointment_type in seed_demo_organization_data
-- Error: "new row for relation appointments violates check constraint appointments_appointment_type_check"
-- Cause: Seed used 'adjustment' but CHECK allows: eye_exam, consultation, fitting, delivery, repair, follow_up, emergency, other
-- Fix: Change 'adjustment' to 'fitting' (Ajuste de lentes)
...
BEGIN
  ...
  -- schedule_settings has partial unique index ON (branch_id) WHERE branch_id IS NOT NULL - must include WHERE in ON CONFLICT
  ...
  -- appointment_type: use 'fitting' not 'adjustment' (CHECK allows: eye_exam, consultation, fitting, delivery, repair, follow_up, emergency, other)
  FOR i IN 1..250 LOOP
  ...
  IF demo_admin_user_id IS NOT NULL AND is_demo_admin THEN
    ticket_num := 1;
    FOR i IN 1..8 LOOP
      INSERT INTO public.optical_internal_support_tickets (organization_id, branch_id, ticket_number, subject, priority, status, created_by, created_at)
      VALUES (demo_org_id, demo_branch_id, 'TKT-DEMO1-' || TO_CHAR(CURRENT_DATE, 'YYYY') || '-' || LPAD(ticket_num::TEXT, 4, '0'), 'Ticket demo ' || ticket_num, (ARRAY['low', 'medium', 'high'])[1 + (i % 3)], CASE WHEN i <= 2 THEN 'open' WHEN i <= 5 THEN 'in_progress' ELSE 'resolved' END, demo_admin_user_id, NOW() - (i || ' days')::INTERVAL);
      ticket_num := ticket_num + 1;
    END LOOP;
    ticket_num := 1;
    FOR i IN 1..6 LOOP
      INSERT INTO public.optical_internal_support_tickets (organization_id, branch_id, ticket_number, subject, priority, status, created_by, created_at)
      VALUES (demo_org_id, demo_branch_2_id, 'TKT-PROV-' || TO_CHAR(CURRENT_DATE, 'YYYY') || '-' || LPAD(ticket_num::TEXT, 4, '0'), 'Ticket Providencia ' || ticket_num, (ARRAY['low', 'medium'])[1 + (i % 2)], CASE WHEN i <= 2 THEN 'open' ELSE 'resolved' END, demo_admin_user_id, NOW() - (i || ' days')::INTERVAL);
      ticket_num := ticket_num + 1;
    END LOOP;
  END IF;
END;

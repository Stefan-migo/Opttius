-- Migration: 20260215100000_seed_demo_full_year.sql
...
-- Migration: Seed Demo Full Year + Branch 2 + Fix Lens Families
-- Creates seed_demo_organization_data() and updates reset_demo_organization()
-- to include second branch (Sucursal Providencia) and full 12-month seed data.
-- Demo org: 00000000-0000-0000-0000-000000000001
-- Branch 1: 00000000-0000-0000-0000-000000000002 (Casa Matriz)
...
BEGIN
  ...
  IF mono_basico_id IS NOT NULL THEN
    ...
    VALUES (mono_basico_id, -4.00, 4.00, -2.00, 2.00, 0.00, 0.00, 29990, 9990, 'stock', true),
           (mono_basico_id, -6.00, -4.25, -2.00, 2.00, 0.00, 0.00, 39990, 14990, 'surfaced', true),
           (mono_basico_id, 4.25, 6.00, -2.00, 2.00, 0.00, 0.00, 39990, 14990, 'surfaced', true);
  END IF;
  IF mono_poly_id IS NOT NULL THEN
    INSERT INTO public.lens_price_matrices (lens_family_id, sphere_min, sphere_max, cylinder_min, cylinder_max, addition_min, addition_max, base_price, cost, sourcing_type, is_active)
    VALUES (mono_poly_id, -4.00, 4.00, -2.00, 2.00, 0.00, 0.00, 59990, 24990, 'stock', true),
           (mono_poly_id, -6.00, -4.25, -4.00, 4.00, 0.00, 0.00, 79990, 34990, 'surfaced', true);
  END IF;
  ...
  IF prog_basico_id IS NOT NULL THEN
    INSERT INTO public.lens_price_matrices (lens_family_id, sphere_min, sphere_max, cylinder_min, cylinder_max, addition_min, addition_max, base_price, cost, sourcing_type, is_active)
    VALUES (prog_basico_id, -4.00, 4.00, -2.00, 2.00, 0.75, 1.75, 99990, 39990, 'surfaced', true),
           (prog_basico_id, -4.00, 4.00, -2.00, 2.00, 2.00, 3.00, 109990, 44990, 'surfaced', true);
  END IF;
  IF prog_comfort_id IS NOT NULL THEN
    INSERT INTO public.lens_price_matrices (lens_family_id, sphere_min, sphere_max, cylinder_min, cylinder_max, addition_min, addition_max, base_price, cost, sourcing_type, is_active)
    VALUES (prog_comfort_id, -4.00, 4.00, -2.00, 2.00, 0.75, 1.75, 189990, 79990, 'surfaced', true),
           (prog_comfort_id, -4.00, 4.00, -2.00, 2.00, 2.00, 3.00, 209990, 89990, 'surfaced', true);
  END IF;
  ...
  VALUES
  ('70000000-0000-0000-0000-000000000001'::uuid, demo_org_id, 'Acuvue Oasys 1-Day', 'Johnson & Johnson', 'daily', 'spherical', 'silicone_hydrogel', 'box_30', 8.50, 14.30, 'Lentes diarios hidrogel silicona.', true, NOW()),
  ...
  SELECT id, demo_org_id, -6.00, -0.50, 0.00, 0.00, 0, 0, 0.00, 0.00, 29990, 14990, true
  FROM public.contact_lens_families WHERE id = '70000000-0000-0000-0000-000000000001'::uuid
  ...
  INSERT INTO public.contact_lens_price_matrices (contact_lens_family_id, organization_id, sphere_min, sphere_max, cylinder_min, cylinder_max, axis_min, axis_max, addition_min, addition_max, base_price, cost, is_active)
  SELECT id, demo_org_id, -6.00, -0.50, -1.75, -0.75, 10, 180, 0.00, 0.00, 39990, 19990, true
  FROM public.contact_lens_families WHERE id = '70000000-0000-0000-0000-000000000002'::uuid
  ...
  IF marcos_cat_id IS NOT NULL AND lentes_sol_cat_id IS NOT NULL AND accesorios_cat_id IS NOT NULL AND servicios_cat_id IS NOT NULL THEN
    ...
    VALUES
    ('20000000-0000-0000-0000-000000000001'::uuid, demo_branch_id, demo_org_id, 'Marco Ray-Ban RB2140', 'marco-ray-ban-rb2140', 'Marco clásico aviador Ray-Ban.', 89900, 45000, marcos_cat_id, 'RB-2140-BLK', 'active', true, true, 'https://images.unsplash.com/photo-1511499767150-a48a237f0083?w=400', NOW()),
    ...
    VALUES
    ('20000000-0000-0000-0000-000000000016'::uuid, demo_branch_id, demo_org_id, 'Estuche Rígido Premium', 'estuche-rigido-premium', 'Estuche rígido para lentes.', 12900, 5000, accesorios_cat_id, 'ACC-EST-RIG', 'active', true, 'https://images.unsplash.com/photo-1586075010923-2dd4570fb338?w=400', NOW()),
    ...
    VALUES
    ('20000000-0000-0000-0000-000000000021'::uuid, demo_branch_id, demo_org_id, 'Reparación de Marco', 'reparacion-marco', 'Servicio de reparación de marco.', 19900, 0, servicios_cat_id, 'SERV-REP-MAR', 'active', true, NOW()),
  ...
  -- ===== 6. PRESCRIPTIONS for both branches =====
  ...
  -- ===== 7. APPOINTMENTS (12 months, both branches) =====
  FOR i IN 1..120 LOOP
    appt_date := CURRENT_DATE - (random() * 365)::INT * INTERVAL '1 day';
  ...
  IF frame_product_id IS NOT NULL AND lens_family_id IS NOT NULL THEN
    quote_num := 1;
    FOR cust_rec IN SELECT c.id FROM public.customers c INNER JOIN public.prescriptions p ON p.customer_id = c.id WHERE c.branch_id = demo_branch_id LIMIT 15
    LOOP
    ...
    LOOP
      SELECT * INTO rx_rec FROM public.prescriptions WHERE customer_id = cust_rec.id ORDER BY prescription_date DESC LIMIT 1;
      INSERT INTO public.quotes (customer_id, branch_id, quote_number, quote_date, expiration_date, prescription_id, frame_product_id, frame_name, frame_brand, frame_model, frame_color, frame_size, frame_sku, frame_price, lens_family_id, lens_type, lens_material, lens_treatments, presbyopia_solution, frame_cost, lens_cost, treatments_cost, labor_cost, subtotal, tax_amount, discount_amount, total_amount, currency, status, notes, created_at)
      VALUES (cust_rec.id, demo_branch_2_id, 'COT-PROV-' || TO_CHAR(CURRENT_DATE, 'YYYY') || '-' || LPAD(quote_num::TEXT, 3, '0'), CURRENT_DATE - (random()*60)::INT, CURRENT_DATE - (random()*60)::INT + INTERVAL '30 days', rx_rec.id, frame_product_id, 'Marco Oakley OO9208', 'Oakley', 'OO9208', 'Negro', '60-16-145', 'OO-9208-BLK', 129900, lens_family_id, 'progressive', 'polycarbonate', ARRAY['anti_reflective'], 'progressive', 65000, 189990, 30000, 15000, 299990, 56998, 0, 356988, 'CLP', CASE WHEN random()>0.5 THEN 'accepted' WHEN random()>0.25 THEN 'sent' ELSE 'draft' END, 'Presupuesto Providencia', NOW() - (random()*60)::INT)
      ON CONFLICT (quote_number) DO NOTHING;
      quote_num := quote_num + 1;
    END LOOP;
  END IF;

  -- ===== 9. LAB WORK ORDERS (both branches) =====
  SELECT id INTO lens_family_id FROM public.lens_families WHERE id = '40000000-0000-0000-0000-000000000002'::uuid;
  IF frame_product_id IS NOT NULL AND lens_family_id IS NOT NULL THEN
    wo_num := 1;
    FOR cust_rec IN SELECT c.id FROM public.customers c INNER JOIN public.prescriptions p ON p.customer_id = c.id WHERE c.branch_id = demo_branch_id LIMIT 12
    LOOP
      SELECT * INTO rx_rec FROM public.prescriptions WHERE customer_id = cust_rec.id ORDER BY prescription_date DESC LIMIT 1;
      ...
      VALUES (cust_rec.id, demo_branch_id, 'TRB-DEMO1-' || TO_CHAR(CURRENT_DATE, 'YYYY') || '-' || LPAD(wo_num::TEXT, 3, '0'), CURRENT_DATE - (random()*90)::INT, rx_rec.id, frame_product_id, 'Marco Oakley OO9208', 'Oakley', 'OO9208', 'Negro', '60-16-145', 'OO-9208-BLK', lens_family_id, 'single_vision', 'polycarbonate', ARRAY['anti_reflective', 'blue_light_filter'], 'none', 65000, 59990, 35000, 15000, 30000, 169990, 32298, 0, 202288, 'CLP', status_list[status_idx], CASE status_list[status_idx] WHEN 'delivered' THEN 'paid' WHEN 'ready_for_pickup' THEN 'partial' ELSE 'pending' END, 'Trabajo demo', NOW() - (random()*90)::INT, NOW() - (random()*85)::INTERVAL, NOW() - (random()*80)::INTERVAL, CASE WHEN status_list[status_idx] = 'delivered' THEN NOW() - (random()*30)::INTERVAL ELSE NULL END)
    ...
    LOOP
      ...
      status_idx := 1 + (random() * 8)::int;
      INSERT INTO public.lab_work_orders (customer_id, branch_id, work_order_number, work_order_date, prescription_id, frame_product_id, frame_name, frame_brand, frame_model, frame_color, frame_size, frame_sku, lens_family_id, lens_type, lens_material, lens_treatments, presbyopia_solution, frame_cost, lens_cost, treatments_cost, labor_cost, lab_cost, subtotal, tax_amount, discount_amount, total_amount, currency, status, payment_status, internal_notes, created_at, ordered_at, sent_to_lab_at, delivered_at)
      VALUES (cust_rec.id, demo_branch_2_id, 'TRB-PROV-' || TO_CHAR(CURRENT_DATE, 'YYYY') || '-' || LPAD(wo_num::TEXT, 3, '0'), CURRENT_DATE - (random()*90)::INT, rx_rec.id, frame_product_id, 'Marco Ray-Ban RB2140', 'Ray-Ban', 'RB2140', 'Negro', '58-14-140', 'RB-2140-BLK', lens_family_id, 'single_vision', 'polycarbonate', ARRAY['anti_reflective'], 'none', 45000, 59990, 30000, 15000, 28000, 149990, 28498, 0, 178488, 'CLP', status_list[status_idx], CASE status_list[status_idx] WHEN 'delivered' THEN 'paid' WHEN 'ready_for_pickup' THEN 'partial' ELSE 'pending' END, 'Trabajo Providencia', NOW() - (random()*90)::INT, NOW() - (random()*85)::INTERVAL, NOW() - (random()*80)::INTERVAL, CASE WHEN status_list[status_idx] = 'delivered' THEN NOW() - (random()*30)::INTERVAL ELSE NULL END)
      ON CONFLICT (work_order_number) DO NOTHING;
      wo_num := wo_num + 1;
    END LOOP;
  END IF;

  -- ===== 10. ORDERS (12 months, both branches) =====
  ord_num := 1;
  FOR i IN 1..80 LOOP
    ord_date := CURRENT_DATE - (random() * 365)::INT * INTERVAL '1 day';
    INSERT INTO public.orders (branch_id, organization_id, order_number, status, total_amount, payment_status, payment_method_type, customer_name, email, subtotal, tax_amount, discount_amount, currency, created_at, updated_at)
    SELECT demo_branch_id, demo_org_id, 'ORD-DEMO1-' || TO_CHAR(ord_date, 'YYYY') || '-' || LPAD(ord_num::TEXT, 4, '0'), CASE WHEN random()>0.1 THEN 'delivered' ELSE 'processing' END, 80000 + (random()*200000)::int, CASE WHEN random()>0.15 THEN 'paid' ELSE 'pending' END, (ARRAY['cash', 'credit_card', 'debit_card', 'installments'])[1 + (random()*3)::int], c.first_name || ' ' || c.last_name, c.email, (80000 + (random()*200000)::int)/1.19, (80000 + (random()*200000)::int)/1.19*0.19, 0, 'CLP', ord_date + INTERVAL '10 hours', ord_date + INTERVAL '10 hours'
    FROM public.customers c WHERE c.branch_id = demo_branch_id ORDER BY random() LIMIT 1
  ...
  FOR i IN 1..80 LOOP
    ord_date := CURRENT_DATE - (random() * 365)::INT * INTERVAL '1 day';
    INSERT INTO public.orders (branch_id, organization_id, order_number, status, total_amount, payment_status, payment_method_type, customer_name, email, subtotal, tax_amount, discount_amount, currency, created_at, updated_at)
    SELECT demo_branch_2_id, demo_org_id, 'ORD-PROV-' || TO_CHAR(ord_date, 'YYYY') || '-' || LPAD(ord_num::TEXT, 4, '0'), CASE WHEN random()>0.1 THEN 'delivered' ELSE 'processing' END, 70000 + (random()*180000)::int, CASE WHEN random()>0.12 THEN 'paid' ELSE 'pending' END, (ARRAY['cash', 'credit_card', 'debit_card'])[1 + (random()*2)::int], c.first_name || ' ' || c.last_name, c.email, (70000 + (random()*180000)::int)/1.19, (70000 + (random()*180000)::int)/1.19*0.19, 0, 'CLP', ord_date + INTERVAL '11 hours', ord_date + INTERVAL '11 hours'
    FROM public.customers c WHERE c.branch_id = demo_branch_2_id ORDER BY random() LIMIT 1
    ON CONFLICT (order_number) DO NOTHING;
    ord_num := ord_num + 1;
  END LOOP;

  -- ===== 11. CASH REGISTER CLOSURES & POS SESSIONS (both branches, if admin exists) =====
...
COMMENT ON FUNCTION public.seed_demo_organization_data() IS 'Seeds demo organization with lens families, products, and 12 months of operational data for both branches.';

-- ===== UPDATE reset_demo_organization TO INCLUDE BRANCH 2 AND CALL SEED =====
...
BEGIN
  ...
  -- Include both branches in all DELETEs
  ...
  INSERT INTO public.customers (id, branch_id, organization_id, first_name, last_name, email, phone, rut, date_of_birth, gender, address_line_1, city, state, postal_code, country, medical_conditions, allergies, last_eye_exam_date, next_eye_exam_due, preferred_contact_method, insurance_provider, is_active, created_at, updated_at)
  SELECT ('10000000-0000-0000-0000-0000' || LPAD(ser.n::text, 12, '0'))::uuid, demo_branch_id, demo_org_id, 'Cliente' || ser.n, 'Demo' || ser.n, 'cliente' || ser.n || '@email.com', '+56 9 0000 000' || ser.n, '99.999.999-' || ser.n, '1990-01-01', CASE WHEN ser.n % 2 = 0 THEN 'male' ELSE 'female' END, 'Calle Demo ' || ser.n, 'Santiago', 'Región Metropolitana', '7500000', 'Chile', ARRAY[]::TEXT[], ARRAY[]::TEXT[], CURRENT_DATE - INTERVAL '1 year', CURRENT_DATE + INTERVAL '1 year', 'email', 'Fonasa', true, NOW() - INTERVAL '1 year', NOW()
  FROM generate_series(6, 25) AS ser(n)
  ON CONFLICT (id) DO NOTHING;

  -- Re-insert customers Branch 2 (different set)
  INSERT INTO public.customers (id, branch_id, organization_id, first_name, last_name, email, phone, rut, date_of_birth, gender, address_line_1, city, state, postal_code, country, medical_conditions, allergies, last_eye_exam_date, next_eye_exam_due, preferred_contact_method, insurance_provider, is_active, created_at, updated_at)
  VALUES
  ...
  INSERT INTO public.customers (id, branch_id, organization_id, first_name, last_name, email, phone, rut, date_of_birth, gender, address_line_1, city, state, postal_code, country, medical_conditions, allergies, last_eye_exam_date, next_eye_exam_due, preferred_contact_method, insurance_provider, is_active, created_at, updated_at)
  SELECT ('11000000-0000-0000-0000-0000' || LPAD(ser.n::text, 12, '0'))::uuid, demo_branch_2_id, demo_org_id, 'ClienteProv' || ser.n, 'Demo' || ser.n, 'prov' || ser.n || '@providencia.cl', '+56 9 6666 666' || ser.n, '11.666.666-' || ser.n, '1988-01-01', CASE WHEN ser.n % 2 = 0 THEN 'female' ELSE 'male' END, 'Av. Providencia ' || (ser.n * 100), 'Providencia', 'Región Metropolitana', '7500000', 'Chile', ARRAY[]::TEXT[], ARRAY[]::TEXT[], CURRENT_DATE - INTERVAL '1 year', CURRENT_DATE + INTERVAL '1 year', 'email', 'Fonasa', true, NOW() - INTERVAL '1 year', NOW()
  FROM generate_series(6, 25) AS ser(n)
  ON CONFLICT (id) DO NOTHING;

  -- Call seed to restore lens families, products, and 12 months of data
  PERFORM public.seed_demo_organization_data();
END;
$$;

COMMENT ON FUNCTION public.reset_demo_organization() IS 'Resets Demo Optica (org 00000000-0000-0000-0000-000000000001) with both branches. Deletes all data, re-inserts org, branches, customers, and calls seed_demo_organization_data() for lens families, products, and 12 months of operational data.';

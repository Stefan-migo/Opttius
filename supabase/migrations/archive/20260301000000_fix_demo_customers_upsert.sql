-- Migration: 20260301000000_fix_demo_customers_upsert.sql
...
-- Migration: Fix seed_demo_mirada_clara customer inserts to use DO UPDATE instead of DO NOTHING
-- Ensures that when reset_demo_organization() runs, existing demo customers (e.g. from Óptica Demo Global
-- or a previous seed) are updated with Mirada Clara data, not skipped.
-- Only affects demo org 00000000-0000-0000-0000-000000000001.
...
COMMENT ON FUNCTION public.seed_demo_mirada_clara() IS 'Seeds Demo Óptica Mirada Clara: org, 2 branches, 6 months progressive data. Uses ON CONFLICT DO UPDATE for customers so reset_demo_organization() always syncs to Mirada Clara data.';

-- Migration: 20260215100006_fix_reset_demo_on_conflict_partial_index.sql
...
-- Migration: Fix ON CONFLICT for partial unique indexes in seed_demo_organization_data
-- Error: "there is no unique or exclusion constraint matching the ON CONFLICT specification"
-- Cause: schedule_settings has partial unique index ON (branch_id) WHERE branch_id IS NOT NULL.
--        PostgreSQL requires the same WHERE clause in ON CONFLICT for partial indexes.
-- Also: appointments uses ON CONFLICT DO NOTHING but has no unique constraint on inserted columns;
--       remove it and use plain INSERT.
...
BEGIN
  ...
  -- No unique constraint on (customer_id, branch_id, appointment_date, appointment_time); use plain INSERT
  FOR i IN 1..250 LOOP
